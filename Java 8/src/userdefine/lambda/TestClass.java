...
public class TestClass {
	...
	public static double integrate(Integrable function, double x1, double x2, int numSlices) {
		...
		for (int i = 0; i < numSlices; i++) {
			sum += delta * function.eval(start + delta * i);			//function.eval again will call mail method line
		}
		return (sum);
	}
	...
	public static void main(String[] args) {
		...
		TestClass.integrationTest(x -> Math.sin(x), 0, Math.PI);
		TestClass.integrationTest(x -> Math.exp(x), 2, 20);
	}
}
